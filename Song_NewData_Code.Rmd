...
data2 <- filter(newdata,Year=="2017")
...
fit4 = lm(Voe ~ factor(Year)+Union+Urbanity+Unemployment+GDP_Per_Capital+Totalpop+StudentpopE+giniE,data = data3)
summary(fit4)
par(mfrow=c(2,2))
plot(fit4)
crPlots(fit4)
...
fit5 = lm(log(Voe) ~ factor(Year)+Union+Urbanity+Unemployment+log(GDP_Per_Capital)+log(Totalpop)+log(StudentpopE)+giniE,data = data3)
summary(fit5)
par(mfrow=c(2,2))
plot(fit5)
crPlots(fit5)
...
fit6 = lm(log(Voe) ~ factor(Year)+Union+Urbanity+Unemployment+log(GDP_Per_Capital)+log(Totalpop)+log(StudentpopE)+giniE+norm_Racial_diversity,data = data3)
summary(fit6)
par(mfrow=c(2,2))
plot(fit6)
crPlots(fit6)
...
summary(fit7)
#1 unit increase in unionization would increase the vocational training amount for exp(0.12983) -> 1.138635 -> 13% larger?
```


```{r}
#Mediation Analysis
model.m <- lmer(norm_Racial_diversity~Union+Urbanity+log(Unemployment)+log(GDP)+log(Totalpop)+(1|Year),data=data3)
model.y <- lmer(log(Voe)~Union+norm_Racial_diversity+Urbanity+log(Unemployment)+log(GDP)+log(Totalpop)+(1|Year),data=data3)
