...
author: "Team IG: Xinci Chen, Yiru Fei, Qi Huang, Shicheng Wang, Kelly Kung"
date: "12/03/2019"
output: html_document
---
  
```{r setup, include=FALSE}
pacman::p_load(readxl,dplyr,tidyverse,ggplot2,plotly,ggpubr,MASS,fitdistrplus,mediation,geepack,conf,car,knitr,gclus,scatterplot3d, ppcor,lme4,quantreg)
```
...
```{r}
#Try to analysis the data in the single year (2010)
...
```{r}
#use student population
fit7 = lm(log(Voe) ~ Union+Unemployment+log(GDP_Per_Capita)+log(StudentpopE)+factor(Year),data = data3)
summary(fit7)
par(mfrow=c(2,2))
plot(fit7)
crPlots(fit7)

#use student population + racial diversity
fit8 = lm(log(Voe) ~ Union+Unemployment+log(GDP_Per_Capita)+log(StudentpopE)+norm_Racial_diversity+factor(Year),data = data3)
summary(fit8)
par(mfrow=c(2,2))
plot(fit8)
crPlots(fit8)
...
fit9 = lm(log(Voe) ~ factor(Year)+Union+Urbanity+Unemployment+log(GDP_Per_Capita)+log(Totalpop),data = data4)
summary(fit9)
par(mfrow=c(2,2))
plot(fit9)
crPlots(fit9)
...
```{r}
#remove urbanity
fit10 = lm(log(Voe) ~ factor(Year)+Union+Unemployment+log(GDP_Per_Capita)+log(Totalpop),data = data3)
summary(fit10)
par(mfrow=c(2,2))
plot(fit10)
crPlots(fit10)
