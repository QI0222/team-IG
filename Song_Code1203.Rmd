...
```{r}
#Mediation Analysis
model.tm <- lm(norm_Racial_diversity~Union+Urbanity+log(Unemployment)+log(GDP_Per_Capita)+log(Totalpop)+factor(Year),data=data3)
model.ty <- lm(log(Voe)~Union+norm_Racial_diversity+Urbanity+log(Unemployment)+log(GDP_Per_Capita)+log(Totalpop)+factor(Year),data=data3)
out.1 <- mediate(model.tm,model.ty,sims=1000,treat = "Union",mediator = "norm_Racial_diversity")
summary(out.1)
plot(out.1)
...
```{r}
#Mediation Analysis
model.sm <- lm(norm_Racial_diversity~Union+Urbanity+log(Unemployment)+log(GDP_Per_Capita)+log(StudentpopE)+factor(Year),data=data3)
model.sy <- lm(log(Voe)~Union+norm_Racial_diversity+Urbanity+log(Unemployment)+log(GDP_Per_Capita)+log(StudentpopE)+factor(Year),data=data3)
out.2 <- mediate(model.sm,model.sy,sims=1000,treat = "Union",mediator = "norm_Racial_diversity")
summary(out.2)
plot(out.2)
#In this case, we find the racial diversity does not have a mediation effect.
```
