...
```{r}
fit4 = lm(log(Voe) ~ factor(Year)+Union+Urbanity+Unemployment+log(GDP_Per_Capita)+log(Totalpop),data = newdata3)
summary(fit4)
par(mfrow=c(2,2))
plot(fit4)
crPlots(fit4)
```
```{r}
fit5 = lm(log(Voe) ~ factor(Year)+Union+Urbanity+Unemployment+log(GDP_Per_Capita)+log(Totalpop)+ norm_Racial_diversity,data = newdata3)
summary(fit5)
par(mfrow=c(2,2))
plot(fit5)
crPlots(fit5)
